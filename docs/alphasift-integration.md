...
- 依赖来源：`requirements.txt` 固定到已验证的 AlphaSift 适配层 commit：`git+https://github.com/ZhuLinsen/alphasift.git@2fc87ee8cd3dcc92350d15f06a73db349cb7e240#egg=alphasift`。该来源覆盖 `alphasift.dsa_adapter` 契约与 `screen/list_strategies/get_status` 调用。
...
context = {
    ...
    "dsa": {
        ...
        "include_news": False,
        "news_max_results": 0,
        "capabilities": ["candidate_context", "realtime_quote", "fundamental_context"],
...
AlphaSift 侧已在 `ZhuLinsen/alphasift@2fc87ee8cd3dcc92350d15f06a73db349cb7e240` 提供 DSA provider context 支持、DSA adapter contract，并支持复用 DSA 的 `LLM_TIMEOUT_SEC`。
...
- `/api/v1/alphasift/screen`：调用适配层 `screen(..., use_llm=True)`，并在调用期间临时注入 DSA 已解析的 LLM 运行环境，同时向适配层传入结构化 LLM/DSA provider 配置；AlphaSift 在 LLM 前只消费轻量 DSA provider context，DSA 返回阶段对最终 Top 候选补新闻并复用已增强字段。适配层缺失或运行时异常返回 `424 + diagnostics` 并保留原始错误边界。
...
打包后的桌面端不依赖运行期 `pip install`：`scripts/build-backend.ps1` 会先执行 `pip install -r requirements.txt`，再校验并收集 `alphasift.dsa_adapter` 进 PyInstaller 产物。发布包默认仍关闭；用户在 Web 设置页开启后会先检查适配层，若打包产物异常缺失，应重建或更新桌面后端。
