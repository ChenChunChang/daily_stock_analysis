...
- 已确认工作树无未合并文件与未解析 merge marker（`rg -n "^(<<<<<<< .+|=======$|>>>>>>> .+)"` 无命中）。
- 本轮复核时间：2026-05-31（Asia/Shanghai）。
- `npm ci` 在当前受限沙箱会触发 `esbuild` 的执行权限异常（`EPERM`），故改为 `npm ci --ignore-scripts` 完成依赖安装；在该环境下已复跑全部 Web 侧验证链路，结论如下：
- 关键后端契约与安装状态链路（本轮重跑）：
